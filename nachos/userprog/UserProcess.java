...
public class UserProcess {
	...
	private int handleCreat(int fname)
	{
		if(fname<0)return -1;
		String name=readVirtualMemoryString(fname,256);
	...
	private int handleOpen(int fname)
	{
		if(fname<0)return -1;
		String name=readVirtualMemoryString(fname,256);
		if(name==null)return -1;
	...
	{
		return 0<=id&&id<16&&descriptors[id]!=null;
	}
	private int handleRead(int id,int fbuf,int size)
	{
		...
		if(writeVirtualMemory(fbuf,buf,0,length)!=length)return -1;
		return length;
	}
	...
	private int handleWrite(int id,int fbuf,int size)
	{
		...
		int length=0;
		byte[] buf=new byte[size];
		length=readVirtualMemory(fbuf,buf,0,size);
		if(length!=size)return -1;
		int count=0;
		count=file.write(buf,0,length);
		if(count==-1)return -1;
		return count;
	}
	...
	private int handleUnlink(int fname){
		if(fname<0)return -1;
		String fileName=readVirtualMemoryString(fname,256);
    ...
    //System.out.print("Syscall: " + syscall + "\n");
	switch (syscall) {
	case syscallHalt:
		return handleHalt();
